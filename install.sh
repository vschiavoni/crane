...
ssh -t 10.0.3.111 "echo 'export LD_LIBRARY_PATH=/home/ubuntu/crane/libevent_paxos/.local/lib:$LD_LIBRARY_PATH' >> ~/.profile"
ssh -t 10.0.3.111 "cd crane/xtern/ ; mkdir obj ; cd obj ; ./../configure --prefix=$XTERN_ROOT/install; make clean; make; make install "
ssh -t 10.0.3.111 "source ~/.profile; cd $MSMR_ROOT/libevent_paxos; ./mk; make clean; make "
ssh -t 10.0.3.111 "source ~/.profile; cd $MSMR_ROOT/tools/criu/ ; wget http://download.openvz.org/criu/criu-1.6.tar.bz2; tar jxvf criu-1.6.tar.bz2"
...
ssh -t 10.0.3.111 "cd criu-1.6 && make -j; export SUDO_ASKPASS=~/pass.sh; sudo -A make install "
ssh -t 10.0.3.111 "source ~/.profile; cd $MSMR_ROOT/apps/apache; ./mk"